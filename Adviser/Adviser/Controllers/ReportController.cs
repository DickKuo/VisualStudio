...
namespace Adviser.Controllers
{
    public class ReportController : BaseLoginController
    {
        ...
        public ActionResult RecordReport()
        {
            ...
            DataTable dt = TradeDAO.GetTradeRecordByDueDayPage(LoginHelper.GetLoginInfo().Adviser.SN, NowTime.ToString(TimeFormat), TimeEnd.ToString(TimeFormat), 300, 1);
        ...
        public ActionResult SearchRecord(ReportRequest _Request)
        {
            ...
            {
                DataTable dt = TradeDAO.GetTradeRecordByDueDayPage(LoginHelper.GetLoginInfo().Adviser.SN, _Request.BeginTime.ToString(TimeFormat), _Request.EndTime.ToString(TimeFormat), 300, _Request.Page == 0 ? 1 : _Request.Page);
                MachResult(dt, TradeModel);
        ...
        {
            ReportViewModels.BounsViewModel ViewModel = new ReportViewModels.BounsViewModel();
            ...
            ViewModel.Page = 1;
            int PageCount = 0;
            ViewModel.ListBouns = BounsDB.GetListBouns(ViewModel.BeginTime.ToString(TimeFormat), ViewModel.EndTime.ToString(TimeFormat), LoginHelper.GetLoginInfo().Adviser.SN, ViewModel.Page, out PageCount);
            ViewModel.MaxPage = PageCount;
            return View(ViewModel);
        }

        [HttpPost]
        public ActionResult BounsSearch(ReportViewModels.BounsViewModel ViewModel)
        {
            BounsDAO BounsDB = new BounsDAO();
            int PageCount = 0;
            ViewModel.Page = 1;
        ...
        {
            ReportViewModels.WeekPointViewModel WeekView = new ReportViewModels.WeekPointViewModel();
        ...
        [HttpPost]
        public ActionResult SearchWeekPoint(ReportViewModels.WeekPointViewModel WeekView)
        {
            WeekPointDAO WeekDB = new WeekPointDAO();
